...
| criterion.com | ⚠️ 现场站 Cloudflare「请验证您是真人」复选框（点击循环无解；早期形态为 JS 重定向到无关影片页如 Elephant Boy） | **但 2026-08-07 奉俊昊卡实测：essay 页现场站 curl 直抓 ✓**（`criterion.com/current/posts/<id>-<slug>` 全文 84-88KB 含作者署名；站内搜索 `criterion.com/search?q=<关键词>` 也是静态 HTML，`/current/posts/` 链接可直接 grep 出）。判定：抓前看字节数，<10KB 即挑战壳 → 转 Wayback `web.archive.org/web/2023/<原URL>`；提取后必须核对标题，货不对板直接弃用 |
...
- **电影台词原文 → Wikiquote**（2026-08-07 重庆森林实测）：`en.wikiquote.org/w/api.php?action=query&format=json&prop=extracts&explaintext=1&titles=<片名>` 拿经典台词集合。注意：非英语片收录的是**英译版**，非原声逐字稿——引用时必须标注「英译版，非原声」，中文通行版台词若未核验逐字原文要如实标注「通行转述」。
...
- **Windows 主机上 search_files 对 `C:/...`/`/c/...` 路径报 "系统找不到指定的路径"**（目录+glob 形式同样失败）→ 精读大文件改回 terminal grep（MSYS 路径可用）；read_file 不受影响。
...
- `references/quote-verification.md` — **研习报告/技法卡片引文全量校验协议**（2026-08-07 未麻轮实测，0 MISS）：三类引文提取（反引号/块引用/行内平衡「」、只认带 [研S#] 的行、表格行跳过）、**语言感知归一化**（日文含假名→去全部空白解决换行劈词；中日英折叠空白；英文源剥 markdown 强调符；弯引号转直）、失配陷阱目录（繁简/引号类型/全角逗号/省略号长度/源站讹误照抄/标签张冠李戴）、译文注疏显式豁免表纪律。可运行样例 `film-suite-research/pages/_verify_kon_pb.py`。
