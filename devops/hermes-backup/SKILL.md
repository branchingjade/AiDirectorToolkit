...
- **EXCLUDE_DIRS 必须含 `node_modules` 和 `.next`**（2026-08-24 实战）：open-design 这类子项目的 pnpm workspace 里有 `node_modules/@open-design/web/.next/dev/lock`，dev server 运行时会持有该文件句柄。Python `tarfile.open` 写入时直接 `PermissionError` 中断整包（不是 warning，是 fatal），导致 8/22/8/23/8/24 连续 3 次早 8:30 cron 备份失败。**根因**=脚本把 dev server 看作「派生可重装」的内容，但排除规则只看了「目录名是不是 node_modules」没看 `.next`，dev server 持有 `.next/dev/lock` 直接命中。**修法**：`EXCLUDE_DIRS` 同时加 `node_modules` 和 `.next`——两者都是 `pnpm install` / `pnpm dev` 可重建的派生产物，**程序本体不备，备份用户数据**这条铁律直接覆盖。**`.git` 不应排除**——git 历史是项目产出的一部分（131MB），可重装不是它。
- **`state.db.pre-update-emergency-*.bak` 必须排除**（2026-08-26 实战，3.4GB 主包根因）：hermes update 失败时会自动备份当前 state.db 为 `state.db.pre-update-emergency-<ISO时间戳>.bak`，单文件 ~2GB。**脚本 `EXCLUDE_FILES` 用精确集合匹配（`name in EXCLUDE_FILES`），通配符不生效**——加 `"state.db.pre-update-emergency-*.bak"` 这种 glob 没用。**修法二选一**：①把 `walk_files` 里的 `if name in EXCLUDE_FILES` 改成 fnmatch 匹配（`any(fnmatch.fnmatch(name, p) for p in EXCLUDE_FILES)`）；②直接在 walk_files 加一行 `if name.startswith("state.db.pre-update-emergency-") and name.endswith(".bak"): continue`——后者更小必要。**残留处理**：修复前留下的 emergency backup 文件**手动删**（`rm -v .../state.db.pre-update-emergency-*.bak`，3 份释放 ~6GB）；KEEP=7 保留逻辑不限制这类文件大小，**会自动逐次备份但永远不清**。**判定**：备份主包 >2GB 而 source 数据无显著变化时，先 `ls -la ~/AppData/Local/hermes/state.db.pre-update-emergency-*.bak` 查 bak 总大小——3 份 × 2GB = 6GB 一次性占走主包 2/3 体积。
- **kill 一个后台 tarfile 备份会留半成品包**：进程被 SIGKILL 时 `tarfile.open` 的写入流没 close，backups/ 目录会留一个几百 MB 的损坏半成品包。跑测试时 kill 后必须删，否则下次脚本跑清理逻辑判断时容易留这个垃圾。下次跑会按保留策略自动覆盖/清理。
- **实际跑的脚本路径必须先 grep**：同一台机器可能有多份 `backup-hermes-*.py`（精简版 vs 全量版）。日志签名识别：`打包完成: X MB (workspace=N文件, hermes=N文件)` 是全量版（Documents/Hermes/scripts/），精简版是 tar.exe 调用，不会出 Python errno 异常。
