...
- **话题消息可能完全不被推送 = 先查 bot 是否在会话里（实测 2026-08-08）**：话题里 @Hermes 有绿色对勾但 gateway 日志该时段零记录（连 "Received raw message" 都没有）时，**第一嫌疑是话题开在用户之间的 p2p 私聊里，bot 不是会话成员**——飞书只推送给会话内成员，非成员 bot 收不到任何消息（话题标题「来自: XX和XX的会话」= p2p 私聊话题的标志）。验证：`lark-cli im +chat-list --as user --types=p2p` 找该会话，`p2p_target_type=user` 即实锤；**最硬验证：`+chat-members-list --as user --chat-id <oc_>` 看 `bot_total: 0` 实锤 bot 非成员**；再 `+chat-messages-list` 拿 `thread_id`、`+threads-messages-list` 确认 @ 已发出。推论（用户常问）：**该话题里 @ 任何非会话成员（人 or bot 一律）都收不到**——@ 只是提醒语义不改变推送范围，私聊话题无第三方入口。修复只能换会话：在 bot 所在的群开话题、直接私聊 bot、或建含 bot 的新群。完整诊断命令链+证据链见 `references/topic-thread-debugging.md` §根因类别二。仅当 bot 确认在会话里仍无推送，才怀疑飞书开发者后台事件订阅。
