...
Feishu/Lark manual setup (bypasses broken interactive wizard on Windows): `skill_view(name="hermes-agent", file_path="references/feishu-lark-setup.md")`
...
**`hermes gateway setup` hangs on Windows PTY.** The interactive curses-style
platform selector doesn't respond to `submit` or `write` through the Hermes
PTY backend — the process gets stuck after entering a choice number. Workaround:
configure platforms manually via environment variables in `~/.hermes/.env`
instead of using the wizard. Each platform's required vars are documented in
the platform docs or in `references/` under this skill (e.g.
`references/feishu-lark-setup.md` for Feishu/Lark).
...
### Installing skills manually (from .skill file or GitHub)
For `.skill` files (ZIP archives) or GitHub-based skill repos not in the hub, see `skill_view(name="hermes-agent", file_path="references/manual-skill-install.md")` — covers cp437 encoding fix for non-ASCII filenames, git-tag-based updates, and the `hermes skills tap add` alternative.
