...
> **v1.0.82 语法变化**：`auth login --scope` 接受**空格分隔的多个 scope**（`--scope "search:docs:read calendar:calendar.event:read"`），不要重复传多个 `--scope` flag（会报 `unknown flag`）。
...
**Bot 创建 docx 的权限链（实测踩坑，2026-08）**：
- 创建文档需要 `docx:document`（**完整权限**）——只有 `docx:document:create` / `docx:document:readonly` 子权限不够，报 `app_scope_not_applied`（code 99991672），missing_scopes 里明确列出 `docx:document`
- 创建成功后 CLI 会尝试自动授予当前用户 `full_access`，需要 `docs:permission.member:create` + `drive:drive` / `drive:file` / `docs:doc` 等 scope；缺则报 Warning（"resource was created, but auto-grant failed"）——**文档已创建但用户看不到**，需补权限后重试
- 开通链接可一次拼多个 scope（逗号 URL 编码为 `%2C`）：`https://open.feishu.cn/page/scope-apply?clientID=<appId>&scopes=s1%2Cs2`
- 开通后无需重发版本，直接重试即可；成功标志是 `permission_grant.status=granted` + `perm=full_access`

**写操作验证铁律**：`block_replace` / `block_delete` 返回 `result: failed`（degrade_code=1011 "produced no document changes"）**不代表没生效**——block 级写操作后旧 block ID 失效，重复用同一 ID 调用就会误报 failed，但第一次调用可能已成功。**判断成功必须重新 fetch 验证实际内容，不能只看返回值**；要继续改同一 block 必须先 fetch 拿新 ID。
