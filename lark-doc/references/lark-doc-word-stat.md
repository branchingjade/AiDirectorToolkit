...
脚本输出的 `word_count` 是含中文标点的复合口径，与用户口头说的"1500-2000 字"不一定同口径。**严格判定时三档都看一眼**：

| 口径 | 统计内容 | 适用 |
|---|---|---|
| 汉字数 | `breakdown.han_chars` | 广电/出版行业惯例；用户说"x 字"时按此口径校验 |
| 含中文标点 | `word_count`（脚本默认） | 通用网页/媒体文案 |
| 全标点 | `word_count + english_punctuations` | 学术/全字符计费场景 |

**判定边界**：当口径 A 严格达标、口径 B 超 ≤5%（如汉字 1819 ✓、含标点 2032 超目标 2000 的 +1.6%）时，**按行业惯例口径接受，不进入第二轮精炼**——再压会损伤内容密度。两轮后任何口径仍有缺口，按上文第 4 条如实汇报，禁止"差一点就过"的注水式收敛。
